...
------------------------------------------------------------------------------
--	Criação da seção ROTAS - Fim Passo 4	--
------------------------------------------------------------------------------

-----------
--	7.1 Cria Tabela ROTAS
-----------
Create Table ROTA(
	ROTA_CODIGO		dm_codi UNIQUE,
	ROTA_NOME		dm_nome,
	ROTA_PRECO		dm_prec,
	ROTA_OBSERVA 	dm_obse,
    D_E_L_E_T_		dm_delt,
    R_E_C_N_O_		dm_recn UNIQUE,
    R_E_C_D_E_L_	dm_rcdl,
	
    Primary Key (ROTA_CODIGO, R_E_C_N_O_)
);

Create Or Replace Function set_recno_rota()
Returns Trigger As $$
Begin
    NEW.R_E_C_N_O_ := Coalesce( ( Select Max(R_E_C_N_O_) From ROTA ), 0) + 1;
     Return NEW;
End;
$$ Language plpgsql;

Create Trigger trg_set_recno_rota
Before Insert On ROTA
For Each Row
Execute Function set_recno_rota();


-----------
--	5.2 Cria Procedure para inclusão de dados na tabela de ROTAS
-----------
Create Or Replace Procedure add_rota(
    ad_ROTA_CODIGO	dm_codi,
	ad_ROTA_NOME	dm_nome,
	ad_ROTA_PRECO	dm_prec,
	ad_ROTA_OBSERVA dm_obse
)
Language plpgsql As $$
Begin
    Insert Into ROTA ( ROTA_CODIGO, ROTA_NOME, ROTA_PRECO, ROTA_OBSERVA )
    Values ( ad_ROTA_CODIGO, ad_ROTA_NOME, ad_ROTA_PRECO, ad_ROTA_OBSERVA );
End
$$;

-----------
--	5.3 Cria Procedure para alterar registro dos dados na tabela de Rotas
-----------
Create Or Replace Procedure alte_rota(
	alte_ROTA_CODIGO	dm_codi,
	alte_ROTA_PRECO		dm_prec,
	alte_ROTA_OBSERVA 	dm_obse
)
Language plpgsql As $$
Begin

	If IsNull( alte_ROTA_PRECO, '' ) <> ''
		Then
        Update ROTA Set ROTA_PRECO = alte_ROTA_PRECO Where ROTA_CODIGO = alte_ROTA_CODIGO;
    End If;
	
	If IsNull( alte_ROTA_OBSERVA, '' ) <> ''
		Then
        Update ROTA Set ROTA_OBSERVA = alte_ROTA_OBSERVA Where ROTA_CODIGO = alte_ROTA_CODIGO;
    End If;
...
-----------
--	5.4 Cria Procedure para "deletar" registro dos dados na tabela de ROTAS
-----------
Create Or Replace Procedure dlt_rota(
	dlt_ROTA_CODIGO		dm_codi
)
Language plpgsql As $$
Begin
    Update ROTA Set D_E_L_E_T_ = '*', R_E_C_D_E_L_ = R_E_C_N_O_ Where ROTA_CODIGO = dlt_ROTA_CODIGO;
End;
$$;

------------------------------------------------------------------------------
--	Fim da parte de ROTAS - Fim Passo 4	--
------------------------------------------------------------------------------
